...
exit($out);
  ...
  */
  function runSearch() {
    ...
    foreach ($columns as $index => $column) {
      //file_put_contents(_LOG_PATH_ . "editAJAX.runSearch.column$index.txt", print_r($column, true));
      if ($column['data'] == 'Delete') $column['data'] = 'id';
      //extract($column, EXTR_PREFIX_ALL, 'col_');
      if (!empty($column['search']['value'])) {
        ...
        $addWhere = "`$tmpName` like '%$tmpSearch%'";
         //if ($tmpSearch == '|_') $addWhere .= " ESCAPE '|'";
       $where[] = $addWhere;
      }
    }
  ...
  */
  function updateAIML() {
    ...
    file_put_contents(_LOG_PATH_ . "editAJAX.updateAIML.form_vars.txt", print_r($form_vars, true));

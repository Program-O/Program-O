<?php
  ...
  ***************************************/
  ...
  $msg = isset($_POST['clearDB']) ? processSync() : '';
...
endScript;
  ...
  $all_bots = getBotList();
  $uploadContent = $template->getSection('SyncAIMLForm');
  $showHelp = $template->getSection('SyncShowHelp');
  ...
  $headerTitle = 'Actions:';
  $pageTitle = 'My-Program O - Synchronize AIML';
  $mainContent = $template->getSection('SynchronizeMain');
  $mainTitle = "Synchronize AIML to use for the bot named $bot_name [helpLink]";
...
<aiml version="1.0.1" xmlns="http://alicebot.org/2001/AIML-1.0.1">';
  ...
  */
  function processSync()
  {
    ...
    if (empty($bot_name))
    {
      $sql = "select `bot_name` from `bots` where `bot_id` = ".$post_vars['bot_id']." limit 1;";
    ...
    if (!file_exists('./synchronize'))
      mkdir('./synchronize');

    if (!is_dir('synchronize'))
    {
      $msg = 'Synchronized directory doesn\'t exist.';
    }
    else
    {
      if ($handle = opendir('./synchronize/'.$bot_name)) {
        while (false !== ($file = readdir($handle)))
        {
          if (($file != ".")
           && ($file != ".."))
          {
            $result = parseAIML($file, file_get_contents('./synchronize/'.$bot_name.'/'.$file), true);

            if ($result)
            {
              $msg = 'There was an error with the file : '.$file ." ". $result;
              break;
            }
          }
        }

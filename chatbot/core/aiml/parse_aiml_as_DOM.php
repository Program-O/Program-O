...
function parseTemplateRecursive(&$convoArr, SimpleXMLElement $element, $level = 0)
{
    ...
    //$HTML_tags = array('a', 'abbr', 'acronym', 'address', 'applet', 'area', 'article', 'b', 'bdo', 'big', 'blockquote', 'br', 'button', 'caption', 'center', 'cite', 'code', 'col', 'colgroup', 'dd', 'del', 'dfn', 'dir', 'div', 'dl', 'dt', 'em', 'fieldset', 'font', 'footer', 'form', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'header', 'hr', 'i', 'iframe', 'img', 'ins', 'kbd', 'label', 'legend', 'ol', 'object', 's', 'section', 'script', 'small', 'span', 'strike', 'strong', 'sub', 'sup', 'table', 'tbody', 'td', 'textarea', 'tfoot', 'th', 'thead', 'tr', 'tt', 'u', 'ul');
    if (!isset($convoArr['HTML_Tags'])) {
        $HTML_tags = file(_CONF_PATH_ . 'HTML_Tags.dat', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
        $convoArr['HTML_Tags'] = $HTML_tags;
    }
    else $HTML_tags = $convoArr['HTML_Tags'];
    $doNotParseChildren = array('li');
    $response = array();

    $parentName = _strtolower($element->getName());

 ...
 * @return the tranformed string
**/
function swapPerson($convoArr,$person = 2, $in) { //2 = swap first with second poerson (e.g. I with you) // otherwise swap with third person
...
function set_wildcards($convoArr)
{
  ...
  if($wildcards!=$aiml_pattern)
  {
    ...
    if (preg_match_all("~$ap~si", $checkagainst, $matches))
    {
      ...
      else runDebug( __FILE__, __FUNCTION__, __LINE__, "Something is not right here.",2);
  }
  return $convoArr;
}
...
function expand_shorthand_to_longhand($convoArr)
{
  ...
  $template = str_replace("<sr>","<sr/>",$template);
  $template = str_replace("<br>","<br/>",$template);
  $template = str_replace("<person2/>","<person2><star/></person2>",$template);
  $template = str_replace("<personf/>","<person><formal><star/></formal></person>",$template);
...
function set_simple(&$convoArr,$index,$value)
{
  ...
  if($index=="topic")
  {
    ...
    $convoArr['client_properties'][$index]=$value;
        if (get_client_property($index) !== 'undefined')
        {
          $sql = "update `client_properties` set `value` = '$value' where `user_id` = $user_id and `bot_id` = $bot_id and `name` = '$name';";
        }
        else{
          $sql = "insert into `client_properties` (`id`, `user_id`, `bot_id`, `name`, `value`)
          values (NULL, $user_id, $bot_id, '$index', '$value');";
        }
...
function url_encode_star($convoArr)
{
  ...
  $encoded_star = urlencode($convoArr['star']);
